...
import { memoryStore } from "@maple-kit/core/testing";
...
/** The store the dev server answers from, with something already in it. */
function seeded(branch: string): StoreConnector {
  const store = memoryStore();
  for (const comment of seedComments(branch)) void store.append(comment);
  return store;
}
